...
using DataLayer.Models.EventManagement.NonPersistent;
...
public interface IEventRepos : IBaseRepos<Event>
{
	...
	Task<EventRegistrationSummary?> GetEventRegistrationSummaryAsync(int eventId);
	Task<List<EventOtherFeeItem>> GetOtherFeeItemsAsync(int eventId);
}

public class EventRepos(IConnectionFactory connectionFactory) : BaseRepos<Event>(connectionFactory, Event.DatabaseObject), IEventRepos
{
    ...
    public async Task<EventRegistrationSummary?> GetEventRegistrationSummaryAsync(int eventId)
    {
		string sql = "EXEC ems.[GetEventRegistrationSummary] @EventId";
        
        using var cn = ConnectionFactory.GetDbConnection()!;

        var obj = (await cn.QuerySingleOrDefaultAsync<EventRegistrationSummary>(sql, new { EventId = eventId }));
