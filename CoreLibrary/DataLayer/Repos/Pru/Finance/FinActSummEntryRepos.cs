...
public interface IFinActSummEntryRepos : IBaseRepos<FinActSummEntry>
{
	
}

public class FinActSummEntryRepos(IDbContext dbContext) : BaseRepos<FinActSummEntry>(dbContext, FinActSummEntry.DatabaseObject), IFinActSummEntryRepos
{

	public override async Task<KeyValuePair<int, IEnumerable<FinActSummEntry>>> SearchNewAsync(
		int pgSize = 0,
		int pgNo = 0,
		...
		List<int>? excludeIdList = null)
	{
		...
		var dataList = (await cn.QueryAsync<FinActSummEntry>(sql, param)).AsList();
		...
		return new KeyValuePair<int, IEnumerable<FinActSummEntry>>(count, dataList);
	}
}