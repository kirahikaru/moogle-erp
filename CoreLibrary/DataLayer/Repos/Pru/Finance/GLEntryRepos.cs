...
public interface IGLEntryRepos : IBaseRepos<GLEntry>
{
	
}

public class GLEntryRepos(IDbContext dbContext) : BaseRepos<GLEntry>(dbContext, GLEntry.DatabaseObject), IGLEntryRepos
{

	public override async Task<KeyValuePair<int, IEnumerable<GLEntry>>> SearchNewAsync(
		int pgSize = 0,
		int pgNo = 0,
		...
		List<int>? excludeIdList = null)
	{
		...
		var dataList = (await cn.QueryAsync<GLEntry>(sql, param)).AsList();
		...
		return new KeyValuePair<int, IEnumerable<GLEntry>>(count, dataList);
	}
}