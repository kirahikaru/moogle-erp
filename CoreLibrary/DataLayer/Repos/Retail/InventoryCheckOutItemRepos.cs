...
public interface IInventoryCheckOutItemRepos : IBaseRepos<InventoryCheckOutItem>
{
	Task<List<InventoryCheckOutItem>> GetByItemIdAsync(int itemId);

	Task<List<InventoryCheckOutItem>> SearchAsync(
...
public class InventoryCheckOutItemRepos(IConnectionFactory connectionFactory) : BaseRepos<InventoryCheckOutItem>(connectionFactory, InventoryCheckOutItem.DatabaseObject), IInventoryCheckOutItemRepos
{
	public async Task<List<InventoryCheckOutItem>> GetByItemIdAsync(int itemId)
    {
        ...
        var dataList = (await cn.QueryAsync<InventoryCheckOutItem>(sql, param)).AsList();

        return dataList;
    }

    public override async Task<List<InventoryCheckOutItem>> QuickSearchAsync(int pgSize = 0, int pgNo = 0, string? searchText = null, List<int>? excludeIdList = null)
    {
        if (pgNo < 0 && pgSize < 0)
        ...
        else
        {
            ...
            sql = sbSql.AddTemplate($";WITH pg AS (SELECT t.Id FROM {DbObject.MsSqlTable} t /**where**/ /**orderby**/ OFFSET @PageSize * (@PageNo - 1) rows FETCH NEXT @PageSize ROW ONLY) " +
                    $"SELECT t.*, ici.*, i.*, l.*, uom.* FROM {DbObject.MsSqlTable} t INNER JOIN pg p ON p.Id=t.Id /**leftjoin**/ /**orderby**/").RawSql;
        }

        using var cn = ConnectionFactory.GetDbConnection()!;

        List<InventoryCheckOutItem> data = (await cn.QueryAsync<InventoryCheckOutItem, InventoryCheckOut, Item, Location, UnitOfMeasure, InventoryCheckOutItem>(sql,
                                        (obj, inventoryCheckOut, item, location, uom) =>
                                        {
                                            obj.CheckOut = inventoryCheckOut;
                                            obj.Item = item;
    ...
    public async Task<List<InventoryCheckOutItem>> SearchAsync(
        int pgSize = 0, int pgNo = 0,
        string? objectCode = null,
        string? objectName = null,
        string? objectNameKh = null,
        ...
        List<string>? mfgCountryCodeList = null)
    {
        ...
        var data = (await cn.QueryAsync<InventoryCheckOutItem, InventoryCheckOut, Item, Location, UnitOfMeasure, InventoryCheckOutItem>(sql,
                                        (obj, inventoryCheckOut, item, location, uom) =>
                                        {
                                            obj.CheckOut = inventoryCheckOut;
                                            obj.Item = item;
        ...
        List<string>? mfgCountryCodeList = null)
    {
        ...
        {
            ObjectType = typeof(InventoryCheckOutItem).Name,
            PageSize = pgSize,
            PageCount = pageCount,
