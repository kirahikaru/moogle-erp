...
public interface IKhCommuneRepos : IBaseRepos<KhCommune>
{
	Task<KhCommune?> GetFullAsync(int id);

	Task<KhCommune?> GetGivenVillageAsync(int khVillageId);
	...
	Task<List<KhCommune>> SearchAsync(
...
public class KhCommuneRepos(IDbContext dbContext) : BaseRepos<KhCommune>(dbContext, KhCommune.DatabaseObject), IKhCommuneRepos
{
	public async Task<KhCommune?> GetFullAsync(int id)
    {
        ...
        sbSql.LeftJoin($"{KhDistrict.MsSqlTable} d ON d.Id=t.KhDistrictId");
        ...
        var dataList = (await cn.QueryAsync<KhCommune, KhDistrict, KhProvince, KhCommune>(sql,
                                (obj, d, p) =>
                                {
                                    if (d != null)
                                        d.Province = p;
	...
	public async Task<KhCommune?> GetGivenVillageAsync(int khVillageId)
    {
        SqlBuilder sbSql = new();
        DynamicParameters param = new();
        sbSql.Where("t.IsDeleted=0");
        sbSql.Where("v.Id=@Id");
        sbSql.LeftJoin($"{KhVillage.MsSqlTable} v ON v.IsDeleted=0 AND v.KhCommuneId=t.Id");
        ...
        return await cn.QuerySingleOrDefaultAsync<KhCommune?>(sql, param);
    }
    ...
    public async Task<List<DropDownListItem>> GetForDropdownSelectFullTextAsync(int? khDistrictId, string? searchText = null)
    {
        ...
        sbSql.LeftJoin($"{KhDistrict.MsSqlTable} cd ON cd.IsDeleted=0 AND cd.Id=cc.KhDistrictId");
        sbSql.LeftJoin($"{KhProvince.MsSqlTable} cp ON cp.IsDeleted=0 AND cp.Id=cd.KhProvinceId");
	...
	public override async Task<KeyValuePair<int, IEnumerable<KhCommune>>> SearchNewAsync(
	...
	public override async Task<List<KhCommune>> QuickSearchAsync(int pgSize = 0, int pgNo = 0, string? searchText = null,
        List<int>? excludeIdList = null)
	{
		if (pgNo < 0 && pgSize < 0)
	...
	public async Task<List<KhCommune>> SearchAsync(
        int pgSize = 0, int pgNo = 0,
        string? objectCode = null,
        string? objectName = null,
        string? nameKh = null,
        ...
        List<int>? countryDistrictIds = null)
    {
        ...
        {
            ObjectType = typeof(KhCommune).Name,
            PageSize = pgSize,
            PageCount = pageCount,
