...
public interface IKhDistrictRepos : IBaseRepos<KhDistrict>
{
	Task<KhDistrict?> GetFullAsync(int id);
	...
	/// <returns></returns>
	Task<KhDistrict?> GetGivenSubAddressAsync(string subAddrObjectTypeName, int subAddrId);

	Task<List<KhDistrict>> SearchAsync(
...
public class KhDistrictRepos(IDbContext dbContext) : BaseRepos<KhDistrict>(dbContext, KhDistrict.DatabaseObject), IKhDistrictRepos
{
	public async Task<KhDistrict?> GetFullAsync(int id)
    {
        ...
        sbSql.LeftJoin($"{KhProvince.MsSqlTable} p ON p.Id=t.KhProvinceId");

        using var cn = DbContext.DbCxn;
        string sql = sbSql.AddTemplate($"SELECT * FROM {DbObject.MsSqlTable} t /**leftjoin**/ /**where**/").RawSql;
        var dataList = (await cn.QueryAsync<KhDistrict, KhProvince, KhDistrict>(sql,
                                (obj, p) =>
                                {
                                    obj.Province = p;
    ...
    public async Task<List<DropDownListItem>> GetForDropdownSelectFullTextAsync(int? khProvinceId, string? searchText = null)
    {
        ...
        sbSql.LeftJoin($"{KhProvince.MsSqlTable} p ON p.IsDeleted=0 AND p.Id=d.KhProvinceId");
    ...
    public async Task<KhDistrict?> GetGivenSubAddressAsync(string subAddrObjectTypeName, int subAddrId)
    {
        SqlBuilder sbSql = new();
        ...
        {
            case nameof(KhCommune):
                {
                    sbSql.LeftJoin($"{KhCommune.MsSqlTable} c ON c.IsDeleted=0 AND c.CambodiaDistrictId=d.Id");
                    sbSql.Where("d.IsDeleted=0");
                    sbSql.Where("c.Id=@Id");
                }
                break;
            case nameof(KhVillage):
                {
                    sbSql.LeftJoin($"{KhCommune.MsSqlTable} c ON c.IsDeleted=0 AND c.CambodiaDistrictId=d.Id");
                    sbSql.LeftJoin($"{KhVillage.MsSqlTable} v ON v.IsDeleted=0 AND v.CambodiaCommuneId=c.Id)");
                    sbSql.Where("d.IsDeleted=0");
                    sbSql.Where("v.Id=@Id");
                }
                break;
        ...
        string sql = sbSql.AddTemplate($"SELECT d.* FROM {KhDistrict.MsSqlTable} d /**leftjoin**/ /**where**/").RawSql;
        return await cn.QuerySingleOrDefaultAsync<KhDistrict>(sql, new { Id = subAddrId });
    }

	public override async Task<KeyValuePair<int, IEnumerable<KhDistrict>>> SearchNewAsync(
		...
		List<int>? excludeIdList = null)
	{
        ...
        sbSql.LeftJoin($"{KhProvince.MsSqlTable} prv ON prv.Id=t.KhProvinceId");
	...
	public override async Task<List<KhDistrict>> QuickSearchAsync(int pgSize = 0, int pgNo = 0, string? searchText = null, List<int>? excludeIdList = null)
	{
		if (pgNo < 0 && pgSize < 0)
		...
		var data = (await cn.QueryAsync<KhDistrict, KhProvince, KhDistrict>(sql,
								(obj, prv) =>
								{
									obj.Province = prv;
									return obj;
	...
	public async Task<List<KhDistrict>> SearchAsync(
        int pgSize = 0, int pgNo = 0,
        string? objectCode = null,
        string? objectName = null,
        string? nameKh = null,
        ...
        List<int>? khProvinceIds = null)
    {
        ...
        {
            ObjectType = typeof(KhDistrict).Name,
            PageSize = pgSize,
            PageCount = pageCount,
