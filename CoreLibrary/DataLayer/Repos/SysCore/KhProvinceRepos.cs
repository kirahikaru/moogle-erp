...
public interface IKhProvinceRepos : IBaseRepos<KhProvince>
{
	...
	/// <returns></returns>
	Task<KhProvince?> GetGivenSubAddressAsync(string subAddrObjectTypeName, int subAddrId);

	Task<List<KhProvince>> SearchAsync(
...
public class KhProvinceRepos(IDbContext dbContext) : BaseRepos<KhProvince>(dbContext, KhProvince.DatabaseObject), IKhProvinceRepos
{
    ...
    public async Task<KhProvince?> GetGivenSubAddressAsync(string subAddrObjectTypeName, int subAddrId)
    {
        SqlBuilder sbSql = new();
        DynamicParameters param = new();
        sbSql.Where("t.IsDeleted=0");
        ...
        {
            case nameof(KhDistrict):
                {
                    sbSql.LeftJoin($"{KhDistrict.MsSqlTable} d ON d.IsDeleted=0 AND d.CambodiaProvinceId=t.Id");
                    sbSql.Where("d.Id = @Id");
                }
        ...
        return await cn.QuerySingleOrDefaultAsync<KhProvince>(sql, new { Id = subAddrId });
    }

	public override async Task<KeyValuePair<int, IEnumerable<KhProvince>>> SearchNewAsync(
		...
		List<int>? excludeIdList = null)
	{
		...
		var dataList = await cn.QueryAsync<KhProvince>(sql, param);
	...
	public async Task<List<KhProvince>> SearchAsync(
        int pgSize = 0, int pgNo = 0,
        string? objectCode = null,
        string? objectName = null,
        string? nameKh = null,
        ...
        string? postalCode = null)
    {
        ...
        List<KhProvince> data = (await cn.QueryAsync<KhProvince>(sql, param)).AsList();
