...
public class OccupationRepos(IDbContext dbContext) : BaseRepos<Occupation>(dbContext, Occupation.DatabaseObject), IOccupationRepos
{
	...
	public override async Task<KeyValuePair<int, IEnumerable<Occupation>>> SearchNewAsync(
		...
		List<int>? excludeIdList = null)
	{
		...
		sbSql.LeftJoin($"{OccupationCategory.MsSqlTable} ctg ON ctg.Id=t.OccpCategoryId");
        ...
        var dataList = await cn.QueryAsync<Occupation, OccupationCategory, Occupation>(sql,
                                (obj, category) =>
                                {
                                    obj.Category = category;
                                    return obj;
