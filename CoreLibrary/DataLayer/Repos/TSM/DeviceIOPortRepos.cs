...
public interface IDeviceIOPortRepos : IBaseRepos<DeviceIOPort>
{
	Task<List<DeviceIOPort>> GetByObjectAsync(string linkedObjType, int linkedObjId);
}

public class DeviceIOPortRepos(IDbContext dbContext) : BaseRepos<DeviceIOPort>(dbContext, DeviceIOPort.DatabaseObject), IDeviceIOPortRepos
{
	public async Task<List<DeviceIOPort>> GetByObjectAsync(string linkedObjType, int linkedObjId)
	{
		...
		var dataList = (await cn.QueryAsync<DeviceIOPort>(sql, param)).AsList();
		return dataList;
	}

	public override async Task<KeyValuePair<int, IEnumerable<DeviceIOPort>>> SearchNewAsync(
		...
		List<int>? excludeIdList = null)
	{
		...
		var dataList = await cn.QueryAsync<DeviceIOPort>(sql, param);
