...
public interface IStorageDriveOptionRepos : IBaseRepos<StorageDriveOption>
{
	Task<List<StorageDriveOption>> GetByDriveAsync(string storageType, int storageDriveId);
}

public class StorageDriveOptionRepos(IDbContext dbContext) : BaseRepos<StorageDriveOption>(dbContext, StorageDriveOption.DatabaseObject), IStorageDriveOptionRepos
{
	public async Task<List<StorageDriveOption>> GetByDriveAsync(string storageType, int storageDriveId)
	{
		...
		var dataList = (await cn.QueryAsync<StorageDriveOption>(sql, param)).AsList();
		return dataList;
	}

	public override async Task<KeyValuePair<int, IEnumerable<StorageDriveOption>>> SearchNewAsync(
		...
		List<int>? excludeIdList = null)
	{
		...
		var dataList = await cn.QueryAsync<StorageDriveOption>(sql, param);
